...
class AdManthan:
 ...
 def generate(self,brief):
  brand=brief.get('brand') or 'Brand'; strategy=self.strategy(brief); ai_scenes=ai_scene_data(brief); fallback=fallback_lines(brief); scenes=[]
  for i,template in enumerate(SCENES,1):
   ...
   scenes.append({'scene':i,'timecode':f'{(i-1)*2.5:.1f}-{i*2.5:.1f}s','story_purpose':ai['story_purpose'],'visual_direction':ai['visual_direction'],'effects':effects,'missing_effects':[],'vo':ai['vo'],'sound_direction':ai['sound_direction'],'sound':['/continuity_bridge'] if i not in (1,6) else (['/riser_short','/nature_ambience'] if i==1 else ['/brand_sting','/voiceover_cta']),'on_screen':ai['on_screen'],'effect_status':'VERIFIED LIBRARY MATCH' if len(effects)==3 else 'PARTIAL LIBRARY MATCH'})
  continuous=' '.join(x['vo'] for x in scenes)
  return {'engine':'KALP AdManthan Python Engine','version':'0.7-ai-creative-vo-composer','execution':'Pyodide/WebAssembly','brand':brand,'product':brief.get('product',''),'objective':strategy['objective'],'duration_seconds':15,'scene_count':6,'library_count':self.library.count(),'strategy':strategy,'continuous_vo':continuous,'script_mode':'AI-composed six-beat continuous VO with deterministic fallback','scenes':scenes,'production_package':{'format':'15-second advertisement','aspect_ratio':'16:9','scene_count':6,'deliverables':['AI intent model','research context','final objective','strategy','AI six-beat continuous VO','storyboard','verified effect selections','sound design','production manifest'],'cta':brief.get('cta','Learn more'),'engine_status':'EXECUTED','brief_driven':True,'research_informed':bool(brief.get('research_context')),'ai_composed':bool(ai_scenes),'raw_intent_excluded_from_creative':True}}
