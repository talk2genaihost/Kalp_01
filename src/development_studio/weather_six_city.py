...
This module is intentionally bounded: it fetches forecast data from the public
Open-Meteo geocoding and forecast APIs and returns normalized records. It does
not create execution authority, persist secrets, or invoke KALP agents/tools.
"""
...
def fetch_six_city_weather(cities: tuple[str, ...] = DEFAULT_CITIES) -> list[WeatherRecord]:
    ...
    for city in cities:
        latitude, longitude, resolved_city, country = _geocode(city)
        url = (
...
def flash_weather(records: list[WeatherRecord]) -> str:
    """Render a compact six-city weather flash suitable for a demo UI/log."""
