import { createAstroRashiRuntime } from "./runtime";
import { demoHoroscopeProvider, unavailableCalculationProvider } from "./demo-provider";
import { translations, rashiList, type Locale } from "./localization";
import type { KundliResult } from "./domain";
...
function text(value: unknown): string | null {
  ...
  if (typeof value === "string" || typeof value === "number") {
    ...
    if (!normalized || normalized.toLowerCase() === "ok") return null;
    return normalized;
  }
...
function mapKundli(payload: unknown): KundliResult {
  ...
  return {
    birthPlace: readPath(data, ["birth_place", "birthPlace", "place", "location"]) ?? "—",
    lagna: readPath(data, ["lagna", "ascendant", "ascendant_details.ascendant", "ascendant_details.sign", "rising_sign"]) ?? "—",
    moonSign: readPath(data, ["moon_sign", "moonSign", "chandra_rashi", "chandra_rashi_details.rashi", "moon_details.sign", "moon.sign"]) ?? "—",
    nakshatra: readPath(data, ["nakshatra_details.nakshatra", "nakshatra", "birth_star", "janma_nakshatra"]) ?? "—",
    tithi: readPath(data, ["tithi", "tithi_details.tithi", "panchang.tithi"]) ?? "—",
    yoga: readPath(data, ["yoga", "yoga_details.yoga", "panchang.yoga"]) ?? "—",
    karana: readPath(data, ["karana", "karana_details.karana", "panchang.karana"]) ?? "—",
    sunSign: readPath(data, ["sun_sign", "sunSign", "surya_rashi", "sun_details.sign", "sun.sign"]) ?? "—",
  } as KundliResult;
}
...
function renderKundli(payload: unknown): void {
  ...
  answer.innerHTML = `
    ...
    <div class="kundli-grid">
      <div class="kundli-item"><span class="kundli-item-label">जन्म स्थान</span><strong>${result.birthPlace}</strong></div>
      <div class="kundli-item"><span class="kundli-item-label">लग्न</span><strong>${result.lagna}</strong></div>
      <div class="kundli-item"><span class="kundli-item-label">चंद्र राशि</span><strong>${result.moonSign}</strong></div>
      <div class="kundli-item"><span class="kundli-item-label">नक्षत्र</span><strong>${result.nakshatra}</strong></div>
      <div class="kundli-item"><span class="kundli-item-label">तिथि</span><strong>${result.tithi}</strong></div>
      <div class="kundli-item"><span class="kundli-item-label">योग</span><strong>${result.yoga}</strong></div>
      <div class="kundli-item"><span class="kundli-item-label">करण</span><strong>${result.karana}</strong></div>
      <div class="kundli-item"><span class="kundli-item-label">सूर्य राशि</span><strong>${result.sunSign}</strong></div>
    </div>
...
function renderWeekly(): void {
  ...
  $("selectedRashiHint").textContent = translations[locale].selectedRashiHint;
}
